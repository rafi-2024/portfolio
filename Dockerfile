...
CMD ["sh", "-c", "DATABASE_URL=$(printf '%s' \"$DATABASE_URL\" | tr -d '\"' | tr -d '\\r'); export DATABASE_URL; node -e \"const raw=process.env.DATABASE_URL||''; let u; try { u=new URL(raw); } catch(e) { console.error('DATABASE_URL is not parseable as a URL. Redacted value:', raw.replace(/\\/\\/[^@]*@/, '//***@').slice(0,80)); throw e; } if(u.protocol!=='postgresql:'&&u.protocol!=='postgres:') throw new Error('DATABASE_URL invalid protocol: '+u.protocol); if(!u.port) throw new Error('DATABASE_URL missing/invalid port - got host '+u.hostname+' with no port. On Render use the fromDatabase connectionString reference, which always includes :5432'); console.log('DATABASE_URL OK:', u.hostname+':'+u.port)\" && ./node_modules/.bin/prisma migrate deploy && exec node server.js"]
